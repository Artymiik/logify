...
const CreateWeb = () => {
  ...
  const { responseServer, statusCode, loader, showErrorApp, handleCreateWeb } =
    useCreateWebContext();

  const createWebSubmit = () => {
    handleCreateWeb(fields.name, fields.description, fields.link);
  };
      ...
      >
        <div className="p-5 bg-[#181622] rounded-lg">
          ...
          <button
            ...
            className="flex justify-center items-center bg-[#853bce] transition py-[10px] w-full h-[43px] mt-5 text-[#fff] text-[15px] font-medium"
            onClick={createWebSubmit}
          >
            {loader ? <Loader /> : "Connect WebSite"}
          </button>
        </div>
      </div>
    </>
